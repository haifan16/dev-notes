...
在 READ COMMITED 和 REPEATABLE READ 下，InnoDB 的 SELECT 操作使用非锁定的一致性读。但是对于快照数据的定义不相同：
...
在上述的配置下，除了外键约束和唯一性检查依然需要的 Gap Lock，其余情况仅使用 Record Lock 进行锁定。但需要牢记的是：上述设置破坏了 ACID 的 I，且对于 replication，可能会导致主从数据的不一致。另外从性能上来看，READ COMMITED 也不会优于默认的 READ REAPEATABLE。
